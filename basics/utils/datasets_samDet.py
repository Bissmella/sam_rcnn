...
from .general_samDet import xyxy2xywh, xywh2xyxy, clean_str
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def collate_fn(batch):
        img, ir, fm, propos, label, path, shapes = zip(*batch)  # transposed  #zjq
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.stack(ir, 0), torch.stack(fm, 0), propos, torch.cat(label, 0), path, shapes #zjq
...
class LoadImagesAndLabels_sr(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            proposals = load_propos(self, index)
            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            ir, _, _ = letterbox(ir, shape, auto=False, scaleup=self.augment) #zjq
        ...
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels)
        propos = torch.from_numpy(propos).to(torch.float32)
        ...
        ir = ir[:, :, ::-1].transpose(2, 0, 1)
        ir = np.ascontiguousarray(ir) #zjq ascontiguousarray函数将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快
        fm = fm.transpose(2, 0, 1)
        return torch.from_numpy(img), torch.from_numpy(ir), torch.from_numpy(fm.copy()), propos, labels_out, self.img_files[index], shapes
...
def load_feature_map(self, index):
    path = self.img_files[index]
    if not "VEDAI_1024" in path:
        path = path.replace("VEDAI/", "VEDAI_1024/")
    path = path.replace("images", "features")
    path = path.replace(".png", ".npy")
    feature_map = np.load(path)
    feature_map = np.transpose(feature_map, (1, 2, 0))
    return feature_map

def load_propos(self, index):
    path = self.img_files[index]
    if not "VEDAI_1024" in path:
        path = path.replace("VEDAI/", "VEDAI_1024/")
    ...
    path = path[:-7] + ".txt"
    with open(path, "r") as file:
        propos = json.load(file)
    if len(propos) > 400:
                    sorted_list = sorted(propos, key=lambda x: x['predicted_iou'], reverse=True)
                    filtered_boxes = sorted_list[:400]
                    propos = [d['bbox'] for d in filtered_boxes]
    else:
...
def random_perspective(img, ir, fm, propos, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if (border_mf[0] != 0) or (border_mf[1] != 0) or (Mm != np.eye(3)).any():  # image changed
        if perspective:
            ...
            fm = cv2.warpAffine(fm, Mm[:2], dsize=(fm_width, fm_height), borderValue=(114, 114, 114))
    ...
    if n:
        ...
        # clip boxes
        xy[:, [0, 2]] = xy[:, [0, 2]].clip(0, 512)  #*changed
        xy[:, [1, 3]] = xy[:, [1, 3]].clip(0, 512)  #changed

        # filter candidates
        i = box_candidates(box1=propos[:, 0:4].T * s, box2=xy.T)
        propos = propos[i]
        propos[:, 0:4] = xy[i]

    return img, ir, fm, propos, targets #zjq
